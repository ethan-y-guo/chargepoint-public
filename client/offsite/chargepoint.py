...
os.mkdir(f'/Users/ethanguo/chargers/{stamp_str}')
...
#        plt.scatter(lat, long, marker='.')
        ...
        with open(f'/Users/ethanguo/chargers/{stamp_str}/{i}_{j}.json', 'w') as writer:
            writer.write(page.text)
#print(T_cnt, T_av, T_stn)

#plt.show()

with open(f'/Users/ethanguo/chargers/{stamp_str}/tkn.txt', 'w') as writer:
    writer.write(f"{coulomb_tkn} {datetime.utcfromtimestamp(int(coulomb_exp)).strftime('%Y-%m-%d %H:%M:%S')}")
    
with open(f'/Users/ethanguo/chargers/{stamp_str}/stats.txt', 'w') as writer:
